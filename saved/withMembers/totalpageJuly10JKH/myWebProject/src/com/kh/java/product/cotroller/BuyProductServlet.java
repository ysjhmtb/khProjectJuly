...
public class BuyProductServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(count==null&&pno==null){
			list = new ProductService().getCartList(mno);
		}else{
			int result = new ProductService().insertCart(Integer.parseInt(pno), mno, Integer.parseInt(count));
			if(0<result){
				CartVo cv = new ProductService().selectLastCart(mno);
				System.out.println(cv.getCno());
				String[] cnos = {String.valueOf(cv.getCno())};
				int deleteCart = new ProductService().deleteCart(cnos);
				list = new ArrayList<CartVo>();
				list.add(cv);
			}else{
				System.out.println("바로결제 중 에러");
			}
		}
