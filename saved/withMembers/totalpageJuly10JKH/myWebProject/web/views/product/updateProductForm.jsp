...
<script src="/mwp/lib/ckeditor/ckeditor.js"></script>
<script src="/mwp/lib/ckeditor/config.js"></script>
<style>
#topBar {
 	margin-bottom:15px; 
...
$(function(){
		...
		console.log($("#category1 option").text());
		$("#category1 option").each(function(index, value){
			if(value.text=="<%=pv.getCategory().split(">")[0] %>"){
				$(this).attr("selected","seleted");
			}
		});
		$("#category2 option").each(function(index, value){
			if(value.text=="<%=pv.getCategory().split(">")[1] %>"){
				$(this).attr("selected","seleted");
			}
		});
...
<div align="center">
<form id="proInfo" method="post" action="/mwp/updateProduct.do?scr=<%=pv.getImg_src() %>" enctype="multipart/form-data">
	<input type="hidden" name="mno" value="<%=member.getMNO() %>"/>
	<input type="hidden" name="pno" value="<%=pv.getPno() %>"/>
	<div class="info">
	...
	<div class="info">
		...
		<input type="text" class="typeText" placeholder="상품명" name="pr_name" id="pr_name" size="65" value="<%=pv.getName() %>"/>
	</div>
	<div class="info">
		<label for="pr_price"><b>가격</b></label>
		<input type="text" class="typeText" placeholder="가격" name="pr_price" id="pr_price" size="65" value="<%=pv.getPrice() %>"/>
	</div>
	<div class="info" align="left" style="margin-left:120px">
		<label for="pr_img"><b>대표이미지</b></label>
		<input type="file" class="typeText" placeholder="<%=pv.getImg_src() %>" name="pr_img" id="pr_img" size="65" value="<%=pv.getImg_src() %>"/>
	</div>
	<div class="info" align="right" style="margin-right:95px;">
		...
		<script>
			...
			<%
				String contentStr = pv.getContent().trim().replaceAll("(\r\n|\r|\n|\n\r)", " ");
			%>
			CKEDITOR.instances.editor1.setData('<%=contentStr %>');
		</script>
	</div>
