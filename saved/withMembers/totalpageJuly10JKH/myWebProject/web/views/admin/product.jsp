...
if(!(member.getM_ID().equals("admin")) || member==null){%>
   ...
   location.href='/nycProject/index.jsp';
   </script>
<% }%>
      ...
      <tr>
         ...
         <input type="hidden" name="bno" id="bno" value="<%=bv.getBno() %>"/>
            <dl class="order_thumb_wrap">
               <dt>
                  <a href='#' target='_blank'><img src="/mwp/<%=new ProductService().selectProduct(bv.getPno()).getImg_src() %>" class="order_thumb" alt="" /></a>
               <%System.out.println(new ProductService().selectProduct(bv.getPno()).getImg_src()); %>
               </dt>
               <dd>
                  <div>
      ...
      <tr>
         ...
         <td class="left goods_delivery_info" rowspan="1">
            <table width="100%" cellpadding="0" cellspacing="0">
               <tr>
                  <td align="center">
                     <a href="/mwp/payM.do?bno=<%=bv.getBno()%>" class="btn">입금완료</a>
                  </td>
               </tr>
            </table>
         </td>
...
<table class="list_table_style" width="100%" border="0" cellpadding="0" cellspacing="0">
   ...
   <thead>
      <tr>
         ...
         <th scope="col">취소</th>
         <th scope="col">상품상태</th>
         <th scope="col" class="goods_delivery_info">처리상태</th>
      </tr>
   </thead>
   <tbody>
   <%if(null!=cList){ %>
