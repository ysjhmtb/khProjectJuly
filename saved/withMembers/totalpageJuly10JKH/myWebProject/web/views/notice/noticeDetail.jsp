...
<%
	...
	NoticeVo noticeNext = (NoticeVo)request.getAttribute("noticeNext");
	NoticeVo noticePre = (NoticeVo)request.getAttribute("noticePre");
	
	String msgNext = null;
	String msgPre = null;
	
	if(noticeNext ==null){
		msgNext = "다음 게시글이 없습니다.";
	}else if(noticePre ==null){
		msgPre = "이전 게시글이 없습니다.";
	}
...
<!-- 본문내용 시작 -->
	...
	<div class="viewbox" >
		...
		<table class="bbsview_table_style" width="100%" cellpadding="0" cellspacing="0" border="0">
			...
			<thead>
				<tr>
					<th scope="col" colspan="6"> <%=notice.getTitle() %>  <img src="http://edge-sample17.firstmall.kr/admin/skin/default/images/board/icon/icon_hot.gif" title="hot" >  </th>
				</tr>
			</thead>
			...
			<tbody>
				<tr>
					...
					<td class="date"><%=notice.getWriteDate() %></td>

					<th scope="row">조회수</th>

					<td class="hit"><%=notice.getCount() %></td>
	...
	<tbody>
		...
		<%if(msgNext == null){ %>
			...
			<td class="left"><span class="hand highlight-link boad_view_btn sbj" ><a href ="/mwp/noticeDetail.do?noticeNo=<%=noticeNext.getNo()%>&currentPage=<%=currentPage%>"><%=noticeNext.getTitle() %></a></span> </td>
			...
			<td class="center"><%=noticeNext.getWriteDate() %></td>
		<%}else{ %>
			<td class="center"><img src="http://edge-sample17.firstmall.kr/data/skin/fruit_puro_gls/images/board/btn/btn_bbs_icon_prev.gif" class="mb3" align="absmiddle" /> 다음글</td>
		...
		<%if(msgPre == null){ %>
			...
			<td class="left"><span class="hand highlight-link boad_view_btn sbj"><a href ="/mwp/noticeDetail.do?noticeNo=<%=noticePre.getNo()%>&currentPage=<%=currentPage%>"><%=noticePre.getTitle() %></a></span></td>
			<td class="right"><img src="http://edge-sample17.firstmall.kr/data/skin/fruit_puro_gls/images/board/icon/icon_admin.gif" id="icon_admin_img" align="absmiddle" style="vertical-align:middle;" /></td>
			<td class="center"><%=noticePre.getWriteDate() %></td>
		<%}else{ %>
			<td class="center"><img src="http://edge-sample17.firstmall.kr/data/skin/fruit_puro_gls/images/board/btn/btn_bbs_icon_next.gif" align="absmiddle" />  이전글</td>
	...
	<ul class="bbsbtn_wrap4">
		...
		<li><button type="button" class="btn_sch" onclick = "noticeListBtn();">목록</button></li>
		<%if(member!=null&&member.getM_NAME().equals("관리자")){ %>
		<li><button type="button" class="btn_sch" onclick = "noticeDelete();">삭제</button></li>
		<li><button type="button" class="btn_sch" onclick = "noticeUpdate();">수정</button></li>
		<%} %>
	</ul>
...
<script type="text/javascript">
	...
	function noticeListBtn(){
		 location.href="/mwp/noticeList.do?currentPage=<%=currentPage%>"; 		
	}
	function noticeDelete(){
		if(confirm("정말 삭제하시겠습니까??") ==true){
			location.href = "/mwp/noticeDelete.do?NoticeNo=<%=notice.getNo()%>";			
		}else{
			return;
		}
	}
	function noticeUpdate(){
		location.href="/mwp/noticeUpdateForm.do?NoticeNo=<%=notice.getNo()%>&currentPage=<%=currentPage%>";
	}
