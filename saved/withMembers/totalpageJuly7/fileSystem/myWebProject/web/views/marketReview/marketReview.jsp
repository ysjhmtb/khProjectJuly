...
<script>
	function movePage(pageNum){
		location.href = "/mwp/reviewList.do?marketNo=<%=marketNo%>&marketName=<%=marketName%>&currentPage="+pageNum;
	}
	
	function marketReviewDetail(reviewNo){
		location.href = "/mwp/reviewDetail.do?currentPage=<%=currentPage%>&marketNo=<%=marketNo%>&marketName=<%=marketName%>&reviewNo="+reviewNo;
	}
	
	function marketReviewList(){
		location.href= "/mwp/marketReviewList.do";
	}
	
	function updateMarketReview(){
		location.href ="/mwp/views/marketReview/marketReviewWriter.jsp?marketName=<%=marketName%>&marketNo=<%=marketNo%>";
	}
</script>
...
<div id="boardlayout">
	...
	<!-- 검색영역 -->
	<div id="bbslist">
		<form name="boardsearch" id="boardsearch" method="post" action="/mwp/reviewSearch.do">
	...
	<ul class="bbsbtn_wrap">
		<li>
			...
			<span class="searchform">
				<input type="text" name="keyword" id="keyword" placeholder="제목 및 내용 검색" value="" title="제목, 내용" size="40" class="input"  />
...
<table class="bbslist_table_style faqlist" width="100%" border="0" cellpadding="0" cellspacing="0" summary="공지사항입니다">
	...
	<tbody>
		...
		for(ReviewVo rv : list){%>
			<tr>
				<td><%=rv.getNo()%></td>
				<td><a onclick = "marketReviewDetail(<%=rv.getNo()%>);" style ="cursor:pointer"><%=rv.getTitle() %> </a></td>
				<td><%=rv.getName() %></td>
...
<!-- //페이징 -->
	...
	<li class="last">
		<button type="button" class="btn_sch" onclick="updateMarketReview();">리뷰등록</button>
		<button type="button" class="btn_sch" onclick="marketReviewList();">마켓목록</button>
	</li>
</ul>
